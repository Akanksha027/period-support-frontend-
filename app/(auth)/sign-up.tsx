...
export default function SignUpScreen() {
  ...
  const handleGoogleSignUp = async () => {
    try {
      if (!startGoogleOAuth) {
        Alert.alert('Error', 'Google sign-up is unavailable right now. Please try again later.');
        return;
      }
      setLoading(true);
      ...
      Alert.alert('Error', err?.errors?.[0]?.message || 'Failed to sign up with Google. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAppleSignUp = async () => {
    try {
      if (!startAppleOAuth) {
        Alert.alert('Error', 'Apple sign-up is unavailable right now. Please try again later.');
        return;
      }
      setLoading(true);
      ...
      Alert.alert('Error', err?.errors?.[0]?.message || 'Failed to sign up with Apple. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onVerifyPress = async () => {
    ...
    try {
      ...
      Alert.alert('Error', err?.errors?.[0]?.message || 'Failed to verify. Please try again.');
    }
  };
